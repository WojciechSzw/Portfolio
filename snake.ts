...
function pauseGame() {
  const pauseBtn = document.querySelector(
    ".game-box__control-panel__pause-icon"
  );
...
function openSettings() {
  ...
  const inputSpeed = document.querySelector<HTMLInputElement>(
    ".horizontal-strap__grid-box__input-speed"
  );
  const inputWidth = document.querySelector<HTMLInputElement>(
    ".horizontal-strap__grid-box__input-width"
  );
...
function changeSettings() {
  ...
  if (newSpeed === undefined || newWidth === undefined) return;
